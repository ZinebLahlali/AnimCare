                ...
                <table class="w-full text-left">
                    <thead>
                        <tr class="bg-gray-50 border-b border-gray-200">
                            ...
                            <th class="px-6 py-4 text-xs font-bold text-gray-400 uppercase">Reason</th>
                    ...
                    <tbody>
                        ...
                        <tr class="border-b border-gray-100 hover:bg-gray-50">
                            <td class="px-6 py-4 text-sm font-semibold text-gray-700 whitespace-nowrap">{{$rendezVous->heure}} AM</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-3">
                                    <img src="{{asset('/storage', $rendezVous->photo)}}" alt="" class="w-8 h-8 rounded-full object-cover" />
                                    <div>
                                        <p class="text-sm font-bold">{{$rendezVous->animal_name}}</p>
                                        <p class="text-xs text-gray-400">{{$rendezVous->user_name}}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <p class="text-xs text-gray-400">{{$rendezVous->date}}</p>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-2">
                                    <!-- <div class="w-2 h-2 rounded-full bg-green-500"></div> -->
                                    <span class="text-sm text-gray-600">{{$rendezVous->statut}}</span>
                                </div>
                            </td>
                        </tr>
                         @endforeach
            ...
            <div class="flex flex-col gap-3">
                ...
                <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                    ...
                    <div class="flex gap-2">
                        <form method="POST" action="{{ route('rendezvous.accept') }}">
                            @csrf
                            <input type="hidden" name="id" value="{{ $notification->data['rendezvous_id'] }}">
                            <button type="submit" class="bg-purple-500 text-white px-2 py-1 rounded">
                                Accept
                            </button>
                        </form>
                        <form method="POST" action="">
                            @csrf
                            <input type="hidden" name="id" value="{{ $notification->data['rendezvous_id'] }}">
                            <button type="submit" class="bg-white text-purple-400  px-2 py-1 rounded border">
                                Cancel
                            </button>
                        </form>                    
                    </div>
